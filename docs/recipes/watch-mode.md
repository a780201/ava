...
AVA sports an intelligent watch mode. It watches for files to change and runs just those tests that are affected.
...
By default AVA watches for changes to the test files, `package.json`, and any other `.js` files. It'll ignore files in certain directories. This list is provided by the [`ignore-by-default`] package. At the time of writing it contains `.git`, `.nyc_output`, `.sass-cache`, `bower_components`,` coverage` and `node_modules`.
...
You can quickly rerun all tests by typing `rs` on the console with a carriage return. [Just like in `nodemon`](https://github.com/remy/nodemon#manual-restarting)!
